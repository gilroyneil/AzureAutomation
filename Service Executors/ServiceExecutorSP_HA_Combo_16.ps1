...
$xsltEnvironmentAndZimory = $($getCurrentScriptLocation.Path + "\ES_CustomerAndEnvironment_Zimory_Transform_16.xsl")
    ...
    {    
        ...
        {        
            if (verifyPlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPBasePlugName)#Check if the last install timestamp exists, and if it does, in the XML from Zimory a newer date?
            {
                ...
                LogInfo "Calling .\ServiceExecutorBase_HA_Combo_SP_16_Azure.ps1;"
                powershell.exe -noprofile -file "Service Executors\ServiceExecutorBase_HA_Combo_SP_16_Azure.ps1" $zimoryConfigurationFile $zimoryOnboardingConfigurationFile $SkipEarlyStepsForDebugging $isDSIInstall "False" $importServersFromXML $preSizeDBsForDevInstall "False" "False" "False" "False"  $amsSessionID #| Out-Null
