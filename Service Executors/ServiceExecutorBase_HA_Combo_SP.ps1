...
#################################################################################################
    ...
    try
    {
        ...
        if ($SkipXMLCreation -eq "False")
        {       
            processSizingSPv3 $xmlMergedConfigFile #Calculate the number of servers etc and then write directly back to the XML.
        }
...
#################################################################################################
    ...
    {   
        ...
        { 
            ...
            {
                logStatusMessageToHTML "HA Configuration XML will be Processed" "[SP BASE]" $amsEnabled    
    ...
    {    
        ...
        {    
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Modifications (MPs)" "[SP BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
            ...
            LogInfo "Calling ConfigureSQLForES.ps1; SQLIntegrationServices"
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Modifications (MPs Agent Start)" "[SP BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
