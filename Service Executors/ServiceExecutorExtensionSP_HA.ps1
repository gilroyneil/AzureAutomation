...
$xmlMergedConfigFile = $($getCurrentScriptLocation.Path + "\Service Executors\XML\SP_HA\MergedTransformed_" + $currentVersion + "_SP_JD.xml")
#This is the XML file which will be used by the scripted install.
$xmlFinalTempConfigFile = $($getCurrentScriptLocation.Path + "\Service Executors\XML\SP_HA\FinalStage1Output_" + $currentVersion + "_SP_JD.xml")
...
#This is the XML file which holds several settings which are specific to the customer/environment (farm number, deployment sever name, media server name, search proxy).
$xmlCustomerEnvironmentTempConfigFile = $($getCurrentScriptLocation.Path + "\Service Executors\XML\SP_HA\MergedCustomerEnvironmentTransformed_" + $currentVersion + "_SP_JD.xml")
...
$xmlCurrentTopologyPrevMinorVersionConfigFileNoPath = "Final_" + $previousVersion + "_SP_JD_CurrentTopology.xml"
...
$xmlFinalConfigFileNoPath = "Final_" + $currentVersion + "_SP_JD_Extension_HA.xml"
...
$xmlServerImportFile = $($getCurrentScriptLocation.Path + "\Server Import XML\" + $oSVersion + "\sp_ha_servers_import.xml")
...
LogStartTracing $("c:\ServiceExecutorESExtensionSP_HA" + $currentDate.ToString() + ".txt") $htmlUpdateFile $htmlAdminPageFile $htmlStatusFile

logStatusMessageToHTML "Configuring the Service Extension" "[SP_HA]" $amsEnabled
...
if ($SkipEarlyStepsForDebugging -eq $false)
{
    ...
    {   
        ...
        logStatusMessageToHTML "Configuration XML Processed" "[SP_HA]" $amsEnabled
    }
    catch
    {
...
#################################################################################################   
    ...
    {          
        ...
        if ($plugData -ne $null)
        {
            logStatusMessageToHTML "The High Availability Extension has already been applied to the Farm. This Extention can only be applied once." "[SP_HA]" $amsEnabled
            loginfo "[SP_HA] The High Availability Extension has already been applied to the Farm. This Extention can only be applied once."
...
#################################################################################################   
    ...
    {   
        ...
        processSizingSPv2Extension $xmlCurrentTopologyConfigFile $xmlFinalConfigFile -hA $true -qpsMultiplier 0
        ...
        if ($ignoreSizingOutputNamesAndImportServerNamesAndIPs -eq $true)
        {
            ...
            logStatusMessageToHTML "Import Existing Servers into Configuration XML" "[SP_HA]" $amsEnabled 
        ...
        updateSQLAliasesToReflectNewDBServers_SP_HA $xmlFinalConfigFile -hA $true       
        ...
        logStatusMessageToHTML "Sizing XML copied from Base Service XML" "[SP_HA]" $amsEnabled
    }
    catch
    {
...
#################################################################################################
    ...
    else
    {
        ...
        try
        {   
            logStatusMessageToHTML "About to provision the new VMs for the Extension" "[SP_HA]" $amsEnabled
                ...
                else
                {
                    ...
                    if ($isDSIInstall -eq $false)
                    {
                        if ($preInstallSPBinariesForAllExtensions -eq $false)
                        {
                            ...
                            logStatusMessageToHTML "Test Install - Reset the VMs" "[SP_HA]" $amsEnabled
                            loginfo "Reset the VMs for HA"
                            executeResetExtServiceVMsNGEnvironment $ngVMServer $getCurrentScriptLocation
                        }
                        else
                        {
                            logStatusMessageToHTML "Test Install - Dont reset VMs as already installed to in Base Service" "[SP_HA]" $amsEnabled
                            loginfo "Reset the VMs for HA"
                        }
                    }
                }
            ...
            logStatusMessageToHTML "Virtual Machines provisioned" "[SP_HA]" $amsEnabled
        }
        catch
        {
            LogRuntimeError "[SP_HA] Error occurred during ensuring that the Deployments are available and contactable. Stop Installation" $_
            exit 1;
        }
        ...
        {   
            ...
            #Stage 2 - now the deployment(s) are running, perform the initial Startup script configuration.
             if ($preInstallSPBinariesForAllExtensions -eq $false)
             {
                    logStatusMessageToHTML "Running Startup Scripts"             "[SP_HA]" $amsEnabled
            ...
            else
            {
                ...
                logStatusMessageToHTML "No need to run all startup scripts as already done in Base Service - just disable firwalls and perform a reboot" "[SP_HA]" $amsEnabled
        ...
        {
            LogRuntimeError "[SP_HA] Error occurred during provisioning Deployments via Zimory API. Stop Installation" $_
            exit;
        }
...
if ($onlyProcessSizingAndNotInstall -eq "False")
{
    ...
    {  
        ...
        if (($ignoreSizingOutputNamesAndImportServerNamesAndIPs -eq $true) -and ($isDSIInstall -eq $false) -AND ($preInstallSPBinariesForAllExtensions -eq $false))
        {
            ...
            $currentSessionDetailID = logStatusMessageToHTML "Configuring Domain membership" "[SP_HA]" $amsEnabled            
        ...
        if ($preInstallSPBinariesForAllExtensions -eq $false)
        {
            ...
            $currentSessionDetailID = logStatusMessageToHTML "Installing SP Binaries" "[SP_HA]" $amsEnabled
        ...
        $currentSessionDetailID = logStatusMessageToHTML "Installing SQL Server" "[SP_HA]" $amsEnabled
        ...
        $currentSessionDetailID = logStatusMessageToHTML "SQL Create 2nd Drive Folders (1 of 1)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        ...
        $currentSessionDetailID = logStatusMessageToHTML "Starting SQL Model DB changes  (Stage 1 of 1)" "[SP_HA]" $amsEnabled
        $stepLogFilePrefix = "Manager_ConfigureSQLForES" 
        $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
        [datetime] $startOperationTime =  get-date  
        ...
        LogStep "Installing SQL Server 2012"
        LogInfo "Calling .\InstallSQLBinaries.ps1;"
        $currentSessionDetailID = logStatusMessageToHTML "Upgrade Editions SQL Server" "[SP_HA]" $amsEnabled
        $stepLogFilePrefix = "Manager_InstallSQLBinaries"   
        $stepLocalLogFilePrefix = "Local_InstallSQLBinaries" 
        [datetime] $startOperationTime =  get-date   
        ...
        [datetime] $startOperationTime =  get-date 
        $currentSessionDetailID = logStatusMessageToHTML "Configuring SQL Always On (Stage 1 of 2)" "[SP_HA]" $amsEnabled
        ...
        $currentSessionDetailID = logStatusMessageToHTML "Stop OWS Service on all SP Servers" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        ...
        $stepLocalLogFilePrefix = "Local_ConfigureAlwaysOn" 
        [datetime] $startOperationTime =  get-date
        logStatusMessageToHTML "Configuring SQL Always On (Stage 2 of 2)" "[SP_HA]" $amsEnabled
        ...
        LogInfo "Calling FinaliseFarm.ps1; Finalise-Farm"
        $currentSessionDetailID = logStatusMessageToHTML "Start OWS Service on all SP Servers" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        $stepLogFilePrefix = "Manager_FinaliseFarm"
        $stepLocalLogFilePrefix = "Local_FinaliseFarm" 
        [datetime] $startOperationTime =  get-date
        ...
        $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Farm Join)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        ...
        $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Service Apps)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        ...
        LogInfo "Calling FinaliseFarm.ps1; Finalise-Farm"
        $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Finalise Farm)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        $stepLogFilePrefix = "Manager_FinaliseFarm"
        $stepLocalLogFilePrefix = "Local_FinaliseFarm" 
        [datetime] $startOperationTime =  get-date
        ...
        $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (SQL Aliases)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        $stepLogFilePrefix = "Manager_CreateFarmAndInitialConfig"
        $stepLocalLogFilePrefix = "Local_CreateFarmAndInitialConfig" 
        [datetime] $startOperationTime =  get-date
        ...
        $currentSessionDetailID = logStatusMessageToHTML "Checking URLs are accesible prior to Force Failover" "[SP_HA]" $amsEnabled
        checkFarmURLsAccessibleSP $xmlFinalConfigFile


        LogStep "Simulating a Core Data Centre Failover for 4 minutes."
        LogInfo "Calling FinaliseFarm.ps1; Finalise-Farm"
        $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (HA Core 2 Failure Test)" "[SP_HA]" $amsEnabled
        #Update the log file prefix
        $stepLogFilePrefix = "Manager_FinaliseFarm"
        $stepLocalLogFilePrefix = "Local_FinaliseFarm" 
        [datetime] $startOperationTime =  get-date
        ...
        $currentSessionDetailID = logStatusMessageToHTML "Checking URLs are accesible post Forced Failover" "[SP_HA]" $amsEnabled
    ...
    catch
    {
     #Log the file contents to AMS.
       ...
       logStatusMessageToHTML "ERROR: Install has failed"    "[SP_HA]" $amsEnabled
...
#Finish the logging.
logStatusMessageToHTML "Finished" "[SP_HA]" $amsEnabled
