...
$SPBasePlugName = "SAO-SP-Base"
$SPHAlugName = "SAO-SP-HA"
...
LogStartTracing $("c:\v3_ServiceExecutorSP" + $currentDate.ToString() + ".txt")  $htmlUpdateFile $htmlAdminPageFile $htmlStatusFile
...
if (($proceedWithInstall) -and ((verifyPlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPBasePlugName)  -or (verifyPlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPHAlugName)))
{
    ...
    if ($amsUrl -ne "")
    {
        ...
        $sessionName = getSessionNameFromZimoryConfiguration $zimoryConfigurationFile $SPBasePlugName
        $shortCustomerName = getShortCustomerNameFromZimoryConfiguration $zimoryConfigurationFile $SPBasePlugName
        $longCustomerName = getLongCustomerNameFromZimoryConfiguration $zimoryConfigurationFile $SPBasePlugName
    ...
    {    
        ...
        if ($arrayOfPlugsToInstall -contains $SPBasePlugName)
        {        
            if (verifyPlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPBasePlugName)#Check if the last install timestamp exists, and if it does, in the XML from Zimory a newer date?
            {
                ...
                LogStep "Configure SP Base Service"
                ...
                if (($importServersFromXML -eq $true) -and ($isDSIInstall -eq $false))
                {
                    ...
                    $dcIP = getDCIPFromZimoryConfiguration $zimoryConfigurationFile $SPBasePlugName
                ...
                logStatusMessageToHTML "Base Plug found in XML. Starting Base and HA Install" "EXECUTOR" $amsEnabled
                LogInfo "Calling .\ServiceExecutorBase_HA_Combo_SP.ps1;"
                powershell.exe -noprofile -file "Service Executors\ServiceExecutorBase_HA_Combo_SP.ps1" $zimoryConfigurationFile $zimoryOnboardingConfigurationFile $SkipEarlyStepsForDebugging $isDSIInstall "False" $importServersFromXML $preSizeDBsForDevInstall "False" "False" "False" "False"  $amsSessionID #| Out-Null
                LogInfo $((processExitCode $LASTEXITCODE "Service Executor Base SP") +  " (Exit Code: " + $LASTEXITCODE + ")")
                if ($LASTEXITCODE -eq 0)
                {
                    ...
                    savePlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPBasePlugName #As the install was successful, record the date of the Plug Data so that we dont re-run the same install again.                    
                }
                else
                {
            ...
            {
                LogInfo $("The Plug Data in Zimory is as old as the plug data for the last successful " + $SPBasePlugName + " install. Do not proceed with install")
            }
        }
        #END STEP 2.1 - Service Executor Base 
        #################################################################################################

       if ($arrayOfPlugsToInstall -contains $SPHAlugName)
        {
            if (verifyPlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPHAlugName)#Check if the last install timestamp exists, and if it does, in the XML from Zimory a newer date?
            {
                ...
                LogInfo "Calling Service Executors\ServiceExecutorExtensionSP_HA.ps1;"
                powershell.exe -noprofile -file "Service Executors\ServiceExecutorExtensionSP_HA.ps1" $SkipEarlyStepsForDebugging $isDSIInstall "False" $SPHAlugName $importServersFromXML $preSizeDBsForDevInstall $amsSessionID #| Out-Null
                LogInfo $((processExitCode $LASTEXITCODE "HA Service Extension") +  " (Exit Code: " + $LASTEXITCODE + ")")
                if ($LASTEXITCODE -eq 0)
                {
                    loginfo "The Install was successful, so save the timestamp of the plug data to file system so that we can use this in logic to not re-run install later"
                    savePlugLastExecutionTimeStampForSuccessfullInstall $zimoryConfigurationFile $getCurrentScriptLocation $SPHAlugName #As the install was successful, record the date of the Plug Data so that we dont re-run the same install again.
                }
                else
                {
                    loginfo "The Install was not successful, so dont save Plug timestamp."
        ...
        #END STEP 2.2 - Service Executor for HA Extension
        #################################################################################################
