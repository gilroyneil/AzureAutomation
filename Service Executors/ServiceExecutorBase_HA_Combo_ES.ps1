...
#New for 3.4.0
#These next 2 entries are the name of the final XML files which are created in steps 1 - 4
$xmlFinalConfigFileHANoPath = "Final_" + $currentVersion + "_ES_JD_BaseService_HA.xml"
$xmlFinalConfigFileHA = $($getCurrentScriptLocation.Path + "\" + $xmlFinalConfigFileHANoPath)
$xmlFinalConfigFileHANoPath_original = "Final_" + $currentVersion + "_ES_JD_BaseService_HA.xml"
...
$xsltStage1 = $($getCurrentScriptLocation.Path + "\Service Executors\XSLT\ESFarmConfigurationTransformStage1.xsl")
$xsltStage2 = $($getCurrentScriptLocation.Path + "\Service Executors\XSLT\ESFarmConfigurationTransformStage2.xsl")
...
logStatusMessageToHTML "Installing the Base Service" "[ES BASE]" $amsEnabled
...
#################################################################################################
    ...
    try
    {
        ...
        { 
           if ($SkipXMLCreation -eq "False")
            {     
                processSizingESv3 $xmlMergedConfigFile #Calculate the number of servers etc and then write directly back to the XML.                
            }
...
#################################################################################################
    ...
    {   
        ...
        {     
            ...
            {
                logStatusMessageToHTML "HA Configuration XML will be Processed" "[ES BASE]" $amsEnabled    
                $xmlFinalConfigFileHATEMP.Save($xmlFinalConfigFileHA)
                processSizingESv2Extension $xmlFinalConfigFile $xmlFinalConfigFileHA -hA $true -qpsMultiplier 0
    ...
    {    
        ...
        if ($localMediaLocation -ne $null -and $localMediaLocation -ne "")
        {
            ...
            #copyMediaToEachServerLPs $xmlFinalConfigFile
        }
        ...
        {    
            ...
            if ($ignoreSizingOutputNamesAndImportServerNamesAndIPs -eq $false)
            {
                ...
                $currentSessionDetailID = logStatusMessageToHTML "Configuring Domain membership" "[ES BASE]" $amsEnabled
            ...
            $currentSessionDetailID = logStatusMessageToHTML "Installing SP Binaries" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            $currentSessionDetailID = logStatusMessageToHTML "Installing SQL Server" "[ES BASE]" $amsEnabled
            #Update the log file prefix
 ...
 #               Sleep (300)
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SQL Create 2nd Drive Folders (1 of 1)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SQL Model DB changes (1 of 1)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"  
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime = get-date                 
...
if (($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "True")  -or ($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "TRUE") -or ($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "true"))
{
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Stage 1 of 4)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Stage 2 of 4)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Stage 3 of 4)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            #################################################################################################
            #NEW! STEP 5.7 - BA Insight first part.
            LogStep "BA Inisght Integration (Previews and search Parts)"
            LogInfo "Calling Manager_ConfigureBAI.ps1; Manager_ConfigureBAI"
            $currentSessionDetailID = logStatusMessageToHTML "BA Insight Integration" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureBAI"
            $stepLocalLogFilePrefix = "Local_ConfigureBAI" 
            [datetime] $startOperationTime =  get-date 
            powershell.exe -noprofile -file "Packages\BAI\Manager_ConfigureBAI.ps1"  $xmlFinalConfigFileNoPath "PreviewServerAndLongComponentsOnly" #| Out-Null                         
            LogInfo $((processExitCode $LASTEXITCODE "Configure BAI") +  " (Exit Code: " + $LASTEXITCODE + ")")
            $LASTEXITCODE = 0
            if ($amsEnabled)
            {
                try
            ...
            #NEW! END STEP 5 - BA Insight First part

            #################################################################################################
            #STEP 5.7 - Finalise farm
            LogStep "Finalizing SharePoint Farm Configuration"
            LogInfo "Calling FinaliseFarm.ps1; Finalise-Farm"
            $currentSessionDetailID = logStatusMessageToHTML "SharePoint Install (Stage 4 of 4)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Moves (Stage 1 of 1)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
            ...
            LogInfo "Calling ConfigureSQLForES.ps1; AllOthersNoModelDB"
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Modifications (Stage 1 of 1)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
            ...
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Modifications (MPs)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
            ...
            LogInfo "Calling ConfigureSQLForES.ps1; SQLIntegrationServices"
            $currentSessionDetailID = logStatusMessageToHTML "SQL DB Modifications (MPs Agent Start)" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_ConfigureSQLForES"
            $stepLocalLogFilePrefix = "Local_ConfigureSQLForES" 
            [datetime] $startOperationTime =  get-date
...
if (($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "True")  -or ($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "TRUE") -or ($xmlFinalConfigFileDOM.Configuration.Install.IsHAInstall -eq "true"))
{
         ...
         #STEP 5.6 - Configure Always On Availability Groups - Step 1 the Shares, which updates the XML. 
            ...
            $currentSessionDetailID = logStatusMessageToHTML "Rebooting all Servers" "[ES BASE]" $amsEnabled
            #Update the log file prefix
            $stepLogFilePrefix = "Manager_FinaliseFarm"            
            $stepLocalLogFilePrefix = "Local_FinaliseFarm"
            [datetime] $startOperationTime =  get-date
        ...
        catch
        {
            ...
            logStatusMessageToHTML "ERROR: Install has failed" "[ES BASE]" $amsEnabled
...
#Finish the logging.
logStatusMessageToHTML "Finished" "[ES BASE]" $amsEnabled
LogEndServiceExecutionTracing
